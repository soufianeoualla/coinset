import * as React from "react";
import type { SVGProps } from "react";
const SvgCoins = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 32 32"
    {...props}
  >
    <path
      fill="currentColor"
      d="M23 11.196V10.5C23 7.365 18.271 5 12 5S1 7.365 1 10.5v5c0 2.611 3.281 4.686 8 5.308v.692c0 3.135 4.729 5.5 11 5.5s11-2.365 11-5.5v-5c0-2.588-3.177-4.665-8-5.304m6 5.304c0 1.652-3.849 3.5-9 3.5q-.7 0-1.385-.046C21.311 18.97 23 17.375 23 15.5v-2.283c3.734.557 6 2.067 6 3.283M9 18.781v-2.973q1.494.194 3 .192a23 23 0 0 0 3-.193v2.974c-.993.147-1.996.22-3 .219a20.4 20.4 0 0 1-3-.219m12-5.04V15.5c0 1.049-1.551 2.175-4 2.859v-2.922c1.614-.39 2.98-.973 4-1.696M12 7c5.151 0 9 1.848 9 3.5S17.151 14 12 14s-9-1.848-9-3.5S6.849 7 12 7m-9 8.5v-1.759c1.02.723 2.386 1.305 4 1.697v2.92c-2.449-.683-4-1.81-4-2.858m8 6v-.521q.492.02 1 .021.727-.001 1.424-.044a15 15 0 0 0 1.576.47v2.933c-2.449-.684-4-1.81-4-2.859m6 3.281V21.8a22.973 22.973 0 0 0 6 .008v2.973c-1.99.292-4.01.292-6 0m8-.422v-2.922c1.614-.39 2.98-.973 4-1.696V21.5c0 1.049-1.551 2.175-4 2.859"
    />
  </svg>
);
export default SvgCoins;
